...
public class RandomHeuristicsNegotiator implements Negotiator {
    ...
    public RandomHeuristicsNegotiator(long seed) {
        rnd = new Random(seed);
    ...
    public RandomHeuristicsNegotiator(long seed, double kickImportance, double ammoImportance, double blastImportance) {
        rnd = new Random(seed);
    ...
    public void makeProposals(int playerIndex, GameState gs, MessageManager manager) {
        ...
        while (madeProposals < Types.NEGOTIATION_PROPOSAL_LIMIT && i < heuristics.length) {
            ...
            if (heuristics[orderedIndexes[i]] > rnd.nextDouble()) {
